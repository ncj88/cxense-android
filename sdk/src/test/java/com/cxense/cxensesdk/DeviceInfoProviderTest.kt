...
class DeviceInfoProviderTest {
    ...
    private val pm: PackageManager = mock {
        on { getPackageInfo(any<String>(), any()) } doReturn PackageInfo().apply { versionName = VERSION }
    ...
    fun getApplicationVersionError() {
        doThrow(PackageManager.NameNotFoundException()).`when`(pm).getPackageInfo(any<String>(), any())
